...
impl EventHandler for Handler {
    ...
    fn guild_member_addition(&self, ctx: Context, _guild_id: GuildId, mut member: Member) {
        ...
        let channel_id = env::var("CHANNEL_ID").unwrap().parse::<u64>().expect("Expected a CHANNEL_ID in the environment");
        let role_id = env::var("ROLE_ID").unwrap().parse::<u64>().expect("Expected a ROLE_ID in the environment");
